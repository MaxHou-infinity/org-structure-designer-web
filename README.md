...
> **v2.0.4** — 组织架构设计与管理工具：本地 Web/Tauri 双端。支持**批量选择移动、Ctrl+F 搜索、组织健康度分析（含自动优化建议）、内置行业模板**，及数据自动保存/项目文件、拖拽操作、虚拟员工、导出 PNG/Excel/诊断报告。
...
### v2.0.4 (2026-08-23)
- **修复**: 画布布局——重写 `calculateTreeLayout`（父部门水平居中于子部门块，统一间距，`countLeaves` 折叠子部门计 1），修复树渲染到视口外、只有部分部门可见
- **新增**: 组织树父→子引导线（连接线），层级关系一眼可见
- **修复**: 导出 PNG 空白（树渲染到巨宽画布外导致）——已修复
- **修复**: 缩放显示 99.99% 浮点误差——`clampZoom`/载入均取整
- **修复**: 搜索——输入中文无法回显/命中（`onClearHighlight` 引用漂移导致 `setQuery('')` 误清空）；改稳定回调 + 只在打开瞬间重置
- **修复**: 负责人选择下拉被下级部门卡遮挡（打开时父卡 z-index 抬高）；右键菜单跑偏到画布右侧（`createPortal` 到 body，脱离 `transform:scale` 锚定系）
- **体验**: 触控板/鼠标交互升级——双指捏合缩放、双指滑动或空白区拖拽平移画布（Shift+空白拖拽仍框选）、画布右下角浮动缩放控件
- **优化**: 左侧菜单栏精简——上传改为"已上传/未上传"状态条，移除侧边栏重复的缩放控件，整体尺寸压缩（w-80→w-64）
- **工程**: 版本号 2.0.3 → 2.0.4；158 单测全绿
