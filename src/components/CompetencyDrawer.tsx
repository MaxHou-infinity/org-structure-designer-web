...
export function CompetencyCapsule({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full border text-[10px] font-semibold ${s.ring} ${s.text}`}
    >
...
function LegendBar() {
  ...
  return (
    <div className="rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-card px-3 py-2">
      <div className="text-[10px] uppercase tracking-wider text-slate-400 mb-1.5">图例</div>
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
        {states.map((st) => (
          <span key={st} className={`inline-flex items-center gap-1 text-[10px] ${COMPETENCY_STYLE[st].text}`}>
        ...
        <span className="inline-flex items-center gap-1 text-[10px] text-slate-500">
          <span className="text-[10px] px-1 rounded bg-amber-100 text-amber-600 font-medium">+N</span>职级差距
        </span>
        <span className="inline-flex items-center gap-1 text-[10px] text-slate-500">
          <span className="text-[10px] px-1 rounded bg-slate-100 text-slate-500 font-medium">–</span>无数据/未评分
        </span>
      </div>
    </div>
  );
...
export function CompetencyDrawer({
  ...
  // L2/L3 员工行渲染（岗位在岗 or 部门直属员工兜底）
  const employeeRows = useMemo(() => {
    if (!selectedDept) return [];
    const positions = selectedDept.positions ?? [];
    if (positions.length > 0) return []; // 有岗位 → 走岗位展开
    return collectDeptEmployees(selectedDept, false).filter((e) => !e.isVirtual);
  }, [selectedDept]);
      ...
      >
        ...
        {onConfirmNotCompetent && emp.positionId && (isCandidate || isConfirmed) && (
          <button
            ...
            className={`shrink-0 text-[10px] px-1.5 py-0.5 rounded-md border font-medium transition-colors ${
              isConfirmed
  ...
  return (
    <div className="fixed inset-0 z-[80]">
      ...
      {/* 抽屉 */}
      <aside className="absolute inset-y-0 right-0 w-[680px] max-w-[94vw] glass border-l border-white/40 shadow-2xl flex flex-col animate-slideInRight">
        {/* 头部 */}
            ...
            <div className="flex gap-3 overflow-x-auto pb-1">
              {l1.map(({ dept, counts, total, pct, unratedRatio }) => (
                <button
                  ...
                  className={`shrink-0 min-w-[200px] rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-card p-4 text-left transition-all hover:shadow-md ${
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-slate-400">{total} 人</span>
                  </div>
                  ...
                  <div className="mt-1.5 flex items-center gap-2 text-[10px]">
                    ...
                    <span className="text-slate-400">未评 {counts.unrated}</span>
                    <span
                      className={`ml-auto font-medium tabular-nums ${
                        unratedRatio >= 0.3 ? 'text-red-600' : 'text-slate-400'
                      }`}
          ...
          {selectedDept && (
            <section>
              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">
                {selectedDept.name} · L2 岗位层 / L3 员工层
              </h3>
              ...
              {(selectedDept.positions ?? []).length > 0 ? (
                <div className="space-y-2">
                  {(selectedDept.positions ?? [])
                    ...
                    .map((pos) => {
                      ...
                      const pct = (n: number) => (total === 0 ? 0 : Math.round((n / total) * 100));
                      const noCompetent =
                        total > 0 && counts.healthy === 0 && counts.warn === 0;
                      const isExpanded = expandedPosIds.has(pos.id);
                      return (
                        <div
                          key={pos.id}
                          className="rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-card overflow-hidden"
                        >
                          <div className="flex items-center gap-1.5 px-3 py-2">
                            ...
                            {noCompetent && (
                              <span className="shrink-0 text-[10px] px-1.5 py-0.5 rounded-full bg-red-50 text-red-600 border border-red-200">
                                无胜任者
                              </span>
                            )}
                          </div>
                ...
                <div className="rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-card p-2">
